...
import com.example.libtest.ioutils.NIOUtils;
import com.google.common.base.Preconditions;
import com.example.libtest.mpegts.Constants;
import com.example.libtest.mpegts.PacketSupport;
...
public class MTSPacket extends PacketSupport {
	...
	public MTSPacket(boolean transportErrorIndicator, boolean payloadUnitStartIndicator, boolean transportPriority, int pid, int scramblingControl, int continuityCounter) {
		super();
		this.buffer = ByteBuffer.allocate(com.example.libtest.mpegts.Constants.MPEGTS_PACKET_SIZE);
	...
	protected void write() {
		...
		if (containsPayload && payload != null) {
			...
			buffer.position(com.example.libtest.mpegts.Constants.MPEGTS_PACKET_SIZE - payloadLength);
			buffer.put(payload);
		}
		buffer.rewind();
		// First byte
