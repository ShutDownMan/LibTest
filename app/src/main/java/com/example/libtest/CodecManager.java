...
public class CodecManager {
    ...
    public synchronized static Codec[] findEncodersForMimeType(String mimeType) {
        ...
        MediaCodecInfo[] regCodecs = new MediaCodecList(MediaCodecList.ALL_CODECS).getCodecInfos();

        // We loop through the encoders, apparently this can take up to a sec (testes on a GS3)
        for(int j = regCodecs.length - 1; j >= 0; j--){
            MediaCodecInfo codecInfo = regCodecs[j];
                    ...
                    try {
                        ...
                        for (int k = 0; k < capabilities.colorFormats.length; k++) {
                            ...
                            for (int supportedColorFormat : SUPPORTED_COLOR_FORMATS) {
                                if (format == supportedColorFormat) {
                                    formats.add(format);
                                }
                            }
                        }
